...
desc = """
    Collect schoology pictures for import
"""
...
def fn_write_error(msg):
    ...
    handler = logging.FileHandler(
        '{0}schoology_error.log'.format(settings.LOG_FILEPATH))
...
def main():
    ...
    filepath = settings.SCHOOLOGY_JPG_OUTPUT
    ...
    try:
        ...
        if retID is None:
            SUBJECT = '[Schoology Picture Application] failed'
        ...
        # fn_write_error("Error in Schoology buildcsv.py, Error = " + e.message)
        SUBJECT = '[Schoology Application] Error'
        BODY = "Error in Schoology Student_Pictures.py, Error = " + e.message
        # sendmail(settings.SCHOOLOGY_TO_EMAIL,settings.SCHOOLOGY_FROM_EMAIL,
        #     BODY, SUBJECT)
        print(SUBJECT, BODY)
