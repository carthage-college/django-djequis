...
SELECT Distinct 
	...
	CASE WHEN PER.acst in ('WD', "WDU", 'WX') THEN 'TRUE' ELSE 'FALSE' 
		END as disabled,    --All but withdrawn students should be "FALSE", 
...
FROM
		(
			...
			OR lv_date > TODAY-3)
			AND acst IN	("GOOD","LOC","PROB","PROC","PROR","READ","RP","SAB",
				"SHAC","SHOC")
			AND subprog not IN ('KUSD', 'UWPK', 'YOP')
			AND CL != 'UP'
			AND ID NOT IN (Select DISTINCT id from prog_enr_rec 
				WHERE CL in ('FF','FN') AND MONTH(TODAY) < 8 
				AND MONTH(TODAY) > 5) 
			AND ID NOT IN (Select DISTINCT id from prog_enr_rec 
				WHERE PROG = 'GRAD' and acst= 'GOOD') --removes double enrollees
						 ) PER					 	 
					...
					LEFT JOIN conc_table CT2 ON CT2.conc = PER.conc2
