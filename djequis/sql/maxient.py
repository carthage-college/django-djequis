DEMOGRAPHIC_DATA = '''
    SELECT DISTINCT
        DIR.id AS Carthage_ID, TRIM(REPLACE(DIR.email, '@carthage.edu', '')) AS Username,
        DIR.lastname AS Last_Name, DIR.firstname AS First_Name,
        DIR.middlename AS Middle_Name, TO_CHAR(PROF.birth_date, '%Y-%m-%d') AS Date_of_Birth,
        CASE TRIM(PROF.sex)
            WHEN 'M' THEN 'Male'
            WHEN 'F' THEN 'Female'
    ...
    FROM
        directory_vw    DIR INNER JOIN  profile_rec     PROF    ON  DIR.id              =   PROF.id
                            LEFT JOIN   race_table      RACE    ON  PROF.ethnic_code    =   RACE.race
                            LEFT JOIN   stu_serv_rec    HSG     ON  DIR.id              =   HSG.id
                                                                AND HSG.yr              =   YEAR(TODAY)
                                                                AND HSG.sess            =   CASE WHEN MONTH(TODAY) = 1 THEN 'RB' WHEN MONTH(TODAY) <= 6 THEN 'RC' ELSE 'RA' END
                            LEFT JOIN   bldg_table      BLDG    ON  HSG.bldg            =   BLDG.bldg
                            LEFT JOIN   stu_serv_rec    HSGLAST ON  DIR.id              =   HSGLAST.id
                                                                AND HSGLAST.yr          =   CASE WHEN MONTH(TODAY) = 1 THEN YEAR(TODAY) - 1 ELSE YEAR(TODAY) END
                                                                AND HSGLAST.sess        =   CASE WHEN MONTH(TODAY) = 1 THEN 'RA' WHEN MONTH(TODAY) <= 3 THEN 'RB' END
                            ...
                            LEFT JOIN   aa_rec          PERM    ON  DIR.id              =   PERM.id
                                                                AND PERM.aa             =   'PERM'
                                                                AND TODAY           BETWEEN PERM.beg_date AND NVL(PERM.end_date, TODAY)
                            LEFT JOIN   aa_rec          CAMP    ON  DIR.id              =   CAMP.id
                                                                AND CAMP.aa             =   'CAMP'
                                                                AND TODAY           BETWEEN CAMP.beg_date AND NVL(CAMP.end_date, TODAY)
                            LEFT JOIN   aa_rec          ICE	    ON  DIR.id              =   ICE.id
                                                                AND ICE.aa              =   'ICE'
                                                                AND TODAY           BETWEEN ICE.beg_date AND NVL(ICE.end_date, TODAY)
                            ...
                            LEFT JOIN   (
                                            ...
                                            FROM
                                                involve_rec PAST_INV INNER JOIN invl_table PAST_IT ON  PAST_INV.invl       =   PAST_IT.invl
