...
STUDENTS = '''
    SELECT DISTINCT 
        TRIM(NVL(bldg_table.txt,'')) || ' ' || TRIM(NVL(stu_serv_rec.room,'')) as unitcode, 
        directory_vw.firstname AS firstname, directory_vw.lastname AS lastname, 
        directory_vw.email AS emailaddress, TRIM(NVL(aa_rec.phone,'')) AS cellphone
    FROM directory_vw
    ...
    AND acad_cal_rec.acyr = CASE WHEN TODAY < TO_DATE(YEAR(TODAY) || '-07-01', '%Y-%m-%d')	THEN MOD(YEAR(TODAY) - 1, 100) || MOD(YEAR(TODAY), 100)
