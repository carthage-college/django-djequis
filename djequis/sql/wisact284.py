...
WIS_ACT_284_SQL = '''
    select distinct
        ...
        CAST(DAY(loan_rec.beg_date) AS VARCHAR(2)) end  as Beginning_Date,
    ...
    where
        (Aid_Record.id > 0) and
        (Aid_Record.fa_yr = CAST(1617 AS char(4))) and
        (Aid_Record.stat in ('A')) and
        (Aid_Record.amt_stat in ('AD')) and
    ...
    where Financial_Aid_Budget_Record.fa_yr = cast(1617 as char(4))
    ...
    inner join id_rec ID_Record on T1.id = ID_Record.id
        where
            (T1.fa_yr = CAST(1617 AS char(4) )) and
            (T1.stat in ('A')) and
            (T1.amt_stat in ('AD')) and
            (T1.amt > 0) and
