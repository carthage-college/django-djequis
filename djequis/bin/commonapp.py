                    ...
                    try:
                        ...
                        with open(filename, 'rb') as f:
                            ...
                            for row in reader:
                                ...
                                if row.get("feeWaiverCode") == '':
                                    paymentMethod = 'CREDIT'
                                    waiverCode = ''
                                else:
                                    paymentMethod = 'WAIVER'
                                    waiverCode = row.get("feeWaiverCode")
                                ...
                                try:
                                    ...
                                    logger.info(q_create_app);
                                except Exception as err:
                                    logger.error(err, exc_info=True)
                                    logger.error(q_create_app)
                                ...
                                scr.write('------------------------------------------------------------------------------------------------------------------\n')
                                scr.write('-- START INSERT NEW STUDENT APPLICATION for: ' + row.get("firstName", 'nofirstname') + ' ' + row.get("lastName", 'nolastname') + ' - ' + str(apptmp_no) +"\n")
                                scr.write('------------------------------------------------------------------------------------------------------------------\n')
                                logger.info('Begin Student Application: {0} {1} {2}'.format(str(apptmp_no), row.get("firstName"), row.get("lastName"))+"\n")
                                # fetch id from app_voucher on mySQL dB (admissions)
                                # insert into app_vouchers_users on mySQL dB
                                if row.get("feeWaiverCode") and row.get("feeWaiverCode") != '':
                                    try:
                                        q_match = '''
                                            SELECT id
                                            FROM app_vouchers
                                ...
                                ################################################
                                if row.get("preferredPhone") == 'Mobile':
                                    if row.get("contactConsent") == 'Y':
                                        contactConsent = 'N'
                                    elif row.get("contactConsent") == 'N':
                                        contactConsent = 'Y'
                                ...
                                ################################################
                                if row.get("alternatePhoneAvailable") and row.get("alternatePhoneAvailable") != '' and row.get("alternatePhoneAvailable") != 'N':
                                    altType = 'CELL'
                                    if row.get("contactConsent") == 'Y':
                                        contactConsent = 'N'
                                    elif row.get("contactConsent") == 'N':
                                ...
                                if row.get("studentStatus") == 'Full Time':
                                    studentStatus = 'TRAD'
                                    intendHoursEnrolled = 16
                                    app_TRAD_list.append(str(apptmp_no));
                                elif row.get("studentStatus") == 'Part Time':
                                    studentStatus = 'TRAD'
                                    intendHoursEnrolled = 16
                                ...
                                preferredStartTerm = row.get("preferredStartTerm")
                                ...
                                try:
                                    ...
                                    INSERT INTO app_admtmp_rec (
                                        ...
                                        app_source, pref_name, parnt_mtlstat,
                                    ...
                                    '''.format(
                                        ...
                                        parnt_mtlstat, live_with, otherLivingSituation,
                                        armedForcesStatus, hsgType,
                                    )
